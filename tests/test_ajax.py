...
def test_litvar(client, app):
	...
	with app.app_context():
		...
		for values in res:
			...
			possible = [ b'<div class="w3-blue w3-ripple w3-padding-16 w3-large w3-center" style="width:100%">No match in Pubmed using LitVar API</div>', b'PubMed IDs of articles citing this variant']	#https://stackoverflow.com/questions/6531482/how-to-check-if-a-string-contains-an-element-from-a-list-in-python/6531704#6531704
			assert any(test in response.get_data() for test in possible)
			#assert b'<div class="w3-blue w3-ripple w3-padding-16 w3-large w3-center" style="width:100%">No match in Pubmed using LitVar API</div>' in response.get_data() or b'PubMed IDs of articles citing this variant' in response.get_data()
...
def test_intervar(client, app):
	...
	with app.app_context():
		...
		for values in res:
			...
			assert response.status_code == 200
			possible = [b'athogenic', b'lassified', b'enign', b'ncertain']	#https://stackoverflow.com/questions/6531482/how-to-check-if-a-string-contains-an-element-from-a-list-in-python/6531704#6531704
			assert any(test in response.get_data() for test in possible)
			#assert b'athogenic' in response.get_data() or b'lassified' in response.get_data() or b'enign' in response.get_data() or b'ncertain' in response.get_data()
...
def test_lovd(client, app):
	...
	with app.app_context():
		...
		curs = db.cursor(cursor_factory=psycopg2.extras.DictCursor)
		curs.execute(
			"SELECT a.genome_version, a.chr, a.g_name, b.c_name FROM variant a, variant_feature b WHERE a.feature_id = b.id AND a.genome_version = 'hg19' ORDER BY random() LIMIT 5"
		)
		res = curs.fetchall()
		for values in res:
