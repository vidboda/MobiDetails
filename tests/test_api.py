...
@pytest.mark.parametrize(('variant', 'key', 'response'), (
    ('NC_000001.10:g.216595579G>A', 'mobidetails_id', '240'),
    ('NC_000001.11:g.216422237G>A', 'mobidetails_id', '240'),
    # ('nc_000001.11:g.216422237G>A', 'mobidetails_id', '240'),
...
@pytest.mark.parametrize(('variant_id', 'key', 'value'), (
    (256, 'cName', 'c.1771T>C'),
    (1, 'hg19PseudoVCF', '1-216420460-C-A'),
))
...
@pytest.mark.parametrize(('new_variant', 'api_key', 'return_key', 'message'), (
    ...
    ('NM_206933.4:c.100C>T', '', 'mobidetails_id', 240),
...
@pytest.mark.parametrize(('variant_ghgvs', 'api_key', 'gene', 'caller', 'return_key', 'message'), (
    ...
    ('NC_000001.11:g.40817273T>G', '', 'KCNQ4', 'cli', 'mobidetails_id', 242),
    ('NC_000001.11:g.40817273T>G', '', 6298, 'cli', 'mobidetails_id', 242),
    ...
    ('NC_000001.10:g.41282945T>G', '', 'KCNQ4', 'cli', 'mobidetails_id', 242),
...
@pytest.mark.parametrize(('rs_id', 'api_key', 'caller', 'return_key', 'message'), (
    ...
    ('rs10012946', '', 'cli', 'NM_006005.3:c.631+256T>C', 244),
...
@pytest.mark.parametrize(('vcf_str', 'genome_version', 'api_key', 'caller', 'return_key', 'message'), (
    ...
    ('1-216247118-C-TAGCTA', 'GRCh38', '', 'cli', 'NM_206933.4:c.2276delinsTAGCTA', 246),
    ('1-216247118-C-TAGCTA', 'hg38', '', 'cli', 'NM_206933.4:c.2276delinsTAGCTA', 246),
    ('1-216420460-C-TAGCTA', 'hg19', '', 'cli', 'NM_206933.4:c.2276delinsTAGCTA', 246),
))
...
@pytest.mark.parametrize(('endpoint', 'to_find', 'http_code'), (
        ...
        (f'create?variant_chgvs=NM_001429.4:c.3069A>G&caller=browser&api_key={api_key}', '/api/variant/257/browser/', 302), # full create url
        (f'create?variant_chgvs=NM_001429.4:c.3069A>G&api_key={api_key}', '/api/variant/257/browser/', 302), # no caller
        (f'create?variant_chgvs=NM_001429.4:c.3069A>G&caller=cli&api_key={api_key}', 257, 200), # cli caller
        (f'create_g?variant_ghgvs=NC_000022.11:g.41152277A>G&gene_hgnc=EP300&caller=browser&api_key={api_key}', '/api/variant/257/browser/', 302), # full create_g hg38
        (f'create_g?variant_ghgvs=NC_000022.11:g.41152277A>G&gene_hgnc=EP300&api_key={api_key}', '/api/variant/257/browser/', 302), # no caller
        (f'create_g?variant_ghgvs=NC_000022.11:g.41152277A>G&gene_hgnc=EP300&caller=cli&api_key={api_key}', 257, 200), # cli caller
        (f'create_g?variant_ghgvs=NC_000022.10:g.41548281A>G&gene_hgnc=EP300&caller=browser&api_key={api_key}', '/api/variant/257/browser/', 302), # full create_g hg19
        ...
        # (f'create_vcf_str?vcf_str=17-7673822-T-TC&caller=browser&api_key={api_key}', b'NM_000546.6:c.797dup', 200), # full vcf_str
        # (f'create_vcf_str?vcf_str=17-7673822-T-TC&api_key={api_key}', b'NM_000546.6:c.797dup', 200), # no caller
        (f'create_vcf_str?vcf_str=17-7673822-T-TC&caller=browser&api_key={api_key}', '/api/variant/258/browser/', 302), # full vcf_str
        (f'create_vcf_str?vcf_str=17-7673822-T-TC&api_key={api_key}', '/api/variant/258/browser/', 302), # no caller
        (f'create_vcf_str?vcf_str=17-7673822-T-TC&caller=cli&api_key={api_key}', b'NM_000546.6:c.797dup', 200), # cli caller
        ...
        (f'create_vcf_str?vcf_strs=17-7673822-T-TC&api_key={api_key}', '/', 302), # create_vcf_str no caller no genome bad param
        ...
        (f'create_vcf_str?vcf_str=17-7673822-T-TC', '/', 302), # create_vcf_str no caller no genome no api key
        (f'create_vcf_str?vcf_str=17-7673822-T-TC&api_key={api_key}s', '/', 302), # create_vcf_str no caller no genome invalid api key
))
def test_get_create_api(client, app, endpoint, to_find, http_code):
     with app.app_context():
        ...
        if http_code == 302:
            ...
            assert to_find == response.headers['Location']
...
@pytest.mark.parametrize(('variant_id', 'acmg_class', 'api_key', 'return_key', 'message'), (
    ...
    (1, 1, '', 'mobidetails_error', 'ACMG class already submitted by this user for this variant'),
